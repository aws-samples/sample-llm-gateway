...
| Provider quirks | For **Bedrock** Chat Completions targets (provider `auth: aws_iam`) only, requests with tools carry `reasoning_effort: "none"` (Bedrock rejects function tools otherwise for GPT-5.x); direct OpenAI and third-party OpenAI-compatible targets do not get this field. Hosted Responses tools such as `web_search` have no cross-provider meaning and are dropped. |
| Dropped params | Sampling parameters that do not map cleanly across all three protocols are not carried across a conversion: `frequency_penalty`, `presence_penalty`, `seed`, `logit_bias`, `logprobs`, `top_k`, and `n` (`n > 1` keeps only the first choice). `temperature`, `top_p`, and stop sequences are carried. |
